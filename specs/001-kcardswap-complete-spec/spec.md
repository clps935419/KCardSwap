...
**Created**: 2025-12-10  
**Status**: Draft  
**Input**: User description: "/speckit.specify 讀取您的憲法文件（Constitution v1.2.0） 自動生成詳細的規格文件，包含以下 10 個核心部分：使用者認證與個人檔案、小卡管理功能、附近的人搜尋、社交功能、聊天與訊息系統、交換流程管理、商業模式細節、API 端點定義、資料庫 Schema、UI/UX 設計指南"
...
- **FR-CARD-001**：每張小卡資料至少必須包含：擁有者 user_id、偶像名稱、團體/團名、專輯名稱、版本/序號、稀有度、狀態（持有/欲交換/已交換）、圖片 URL、縮圖 URL、建立時間與最後更新時間。
...
- **FR-CARD-004**：系統必須為每張小卡自動產生縮圖（例如 200x200）並在列表畫面優先載入縮圖，以節省頻寬與加快載入。
...
- **FR-CHAT-002**：當使用者停留在特定聊天室畫面前景時，App 必須每 3–5 秒呼叫輪詢 API 取得新訊息，並根據最後讀取時間做增量載入。
