...
**Created**: 2025-12-10  
**Status**: Draft  
**Input**: User description: "/speckit.specify 讀取您的憲法文件（Constitution v1.2.0） 自動生成詳細的規格文件，包含以下 11 個核心部分：使用者認證與個人檔案、小卡管理功能、附近的人搜尋、社交功能、聊天與訊息系統、交換流程管理、商業模式細節、API 端點定義、資料庫 Schema、UI/UX 設計指南、城市看板貼文"
...
### User Story 7 - 城市看板發起交換貼文 (Priority: P2)

使用者可以在指定城市的「城市看板」發起交換貼文（例如「我在台北市，想換某偶像版本」），吸引同城市的人互相聯絡交換。貼文可設定到期時間；其他使用者可對貼文表達「有興趣」，貼文作者接受後系統建立好友關係並建立一對一聊天室供協商交換。

**Why this priority**: 這是以地點為核心的增量獲客/促成交換場景，能補足「僅好友內交換」的不足，但可在既有 AUTH/SOCIAL/CHAT 基礎上循序加入，屬 P2。

**Independent Test**: 僅實作此流程時，測試者可以 A 建立台北市城市看板貼文 → B 在台北市城市看板看到並點選「有興趣」→ A 接受後自動建立聊天室 → 雙方可互傳訊息，即可視為可獨立驗收。

**Acceptance Scenarios**:

1. **Given** 使用者已登入，**When** 在「台北市」城市看板建立貼文並設定 14 天到期，**Then** 貼文出現在該城市看板列表中，且不顯示精確地址。
2. **Given** 使用者位於台北市，**When** 開啟城市看板並以偶像/團體篩選，**Then** 可看到符合條件的貼文列表，並可查看貼文詳情。
3. **Given** 使用者 B 對貼文點選「有興趣」，**When** 貼文作者 A 接受該興趣請求，**Then** 系統建立 A 與 B 的好友關係並建立一對一聊天室，雙方可在聊天室內協商交換。
4. **Given** 貼文已到期或作者手動關閉，**When** 其他使用者瀏覽城市看板，**Then** 該貼文不再出現在預設列表中。
...
- **[ADDED: 2025-12-29] FR-CARD-003A**：系統必須提供「確認上傳」能力以避免 Signed URL 直傳造成幽靈資料：
  - 後端必須提供 `POST /api/v1/cards/{card_id}/confirm-upload`，由客戶端在成功 PUT 到 GCS 後呼叫。
  - 後端在 confirm 時必須驗證該物件確實存在於 GCS（例如以 HEAD/metadata 檢查），驗證成功後將卡片標記為「已完成上傳」。
  - 若物件不存在（上傳失敗/過期/路徑不符），必須回傳可辨識的錯誤（建議 409/422 類型）以引導客戶端重新取得 Signed URL 並重傳。
  - 卡片必須具備 `upload_status`（例如 pending/confirmed）與 `upload_confirmed_at`（或等價欄位），供後端與前端區分是否已完成上傳。
...
- **FR-NEARBY-002**：搜尋結果中不得顯示精確地址或精準座標，只能顯示大約距離與縣市資訊（例如「台北市」）。
...
- **FR-SOCIAL-005A**：系統必須提供「解除封鎖」能力：
  - 提供解除封鎖 API（建議 `POST /api/v1/friends/unblock` 或等價路由），將被封鎖關係移除或恢復為可互動狀態。
  - 解除封鎖後，雙方可再次互動（送出好友邀請/聊天），但不自動成為好友。
  - 前端需提供對應的解除封鎖入口與狀態提示。
...
- **FR-POST-001**：系統必須允許使用者在指定城市（縣市）建立城市看板貼文，貼文至少包含：作者 user_id、city_code、標題、內容、狀態（open/closed/expired/deleted）、到期時間、建立/更新時間。
- **FR-POST-002**：系統必須提供以城市（縣市）為主的城市看板列表；查詢時 `city_code` 必填；列表需支援分頁且預設依建立時間由新到舊排序。
...
- **Post**：城市看板貼文，包含作者、city_code、內容、狀態與到期時間。
