...
type CommentWithPending = CommentResponse & { pending?: boolean }
...
export function useCreateComment(postId: string) {
  ...
  return useMutation({
    ...
    onMutate: async (content: string) => {
      ...
      queryClient.setQueryData<CommentListResponseWrapper>(['post-comments', postId], old => {
        if (!old?.data) return old

        const optimisticComment: CommentWithPending = {
          id: `temp-${Date.now()}`,
          post_id: postId,
          user_id: 'current-user', // Will be replaced with actual user ID from server
    ...
    onSuccess: newComment => {
      ...
      queryClient.setQueryData<CommentListResponseWrapper>(['post-comments', postId], old => {
        ...
        const comments = (old.data.comments || []) as CommentWithPending[]
        // Remove the optimistic comment (marked with pending: true)
        const filteredComments = comments.filter(comment => !comment.pending)
