...
class RateUserUseCase:
    ...
    async def execute(
        ...
        is_blocked = await self.friendship_repository.is_blocked(
            rated_user_id, rater_id
        )
        is_blocker = await self.friendship_repository.is_blocked(
        ...
        if trade_id:
            has_rated = await self.rating_repository.has_user_rated_trade(
                rater_id, trade_id
            )
            if has_rated:
