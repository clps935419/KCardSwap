...
class SQLAlchemyFriendshipRepository(FriendshipRepository):
    ...
    async def create(self, friendship: Friendship) -> Friendship:
        ...
        model = FriendshipModel(
            ...
            user_id=UUID(friendship.user_id) if isinstance(friendship.user_id, str) else friendship.user_id,
            friend_id=UUID(friendship.friend_id) if isinstance(friendship.friend_id, str) else friendship.friend_id,
            status=friendship.status.value if isinstance(friendship.status, FriendshipStatus) else friendship.status,
    ...
    async def update(self, friendship: Friendship) -> Friendship:
        ...
        result = await self.session.execute(
            select(FriendshipModel).where(
                FriendshipModel.id == (UUID(friendship.id) if isinstance(friendship.id, str) else friendship.id)
            )
        )
        model = result.scalar_one_or_none()
