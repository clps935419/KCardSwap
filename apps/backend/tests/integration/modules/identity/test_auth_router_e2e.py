...
class TestAuthRouterE2E:
    ...
    @patch(
        "app.modules.identity.application.use_cases.auth.login_with_google.GoogleOAuthService"
    )
    def test_google_login_success_new_user(
        self, mock_google_service, unauthenticated_client
    ):
        """Test successful Google login for new user (implicit flow)"""
        # Mock Google OAuth service
        mock_service_instance = MagicMock()
        mock_service_instance.verify_google_token = AsyncMock(
            return_value={
        ...
        payload = {"google_token": "valid_google_id_token_12345"}

        response = unauthenticated_client.post(
            "/api/v1/auth/google-login", json=payload
        )
    ...
    def test_google_login_success_existing_user(
        ...
        payload = {"google_token": "valid_google_id_token_67890"}
    ...
    def test_google_login_invalid_token(self, unauthenticated_client):
        ...
        payload = {"google_token": "invalid_token"}
    ...
    def test_google_callback_success(self, mock_google_service, unauthenticated_client):
        ...
        response = unauthenticated_client.post(
            "/api/v1/auth/google-callback", json=payload
        )
    ...
    def test_google_callback_missing_code_verifier(self, unauthenticated_client):
        ...
        payload = {"code": "4/0AY0e-g7XXXXXXXXXXX"}
