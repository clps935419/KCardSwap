...
def upgrade() -> None:
    ...
    indexes = [idx['name'] for idx in inspector.get_indexes('friendships')] if 'friendships' in tables else []
    ...
    indexes = [idx['name'] for idx in inspector.get_indexes('chat_rooms')] if 'chat_rooms' in tables else []
    ...
    indexes = [idx['name'] for idx in inspector.get_indexes('messages')] if 'messages' in tables else []
    ...
    if 'idx_message_room_created' not in indexes:
        op.create_index('idx_message_room_created', 'messages', ['room_id', 'created_at'])
    ...
    if 'idx_message_status_sender' not in indexes:
        op.create_index('idx_message_status_sender', 'messages', ['room_id', 'status', 'sender_id'])
    ...
    indexes = [idx['name'] for idx in inspector.get_indexes('ratings')] if 'ratings' in tables else []
    ...
    if 'idx_rating_trade_rater' not in indexes:
        op.create_index('idx_rating_trade_rater', 'ratings', ['trade_id', 'rater_id'], unique=True)
    ...
    indexes = [idx['name'] for idx in inspector.get_indexes('reports')] if 'reports' in tables else []
    ...
    if 'idx_report_resolved_created' not in indexes:
        op.create_index('idx_report_resolved_created', 'reports', ['resolved', 'created_at'])
    
    if 'idx_report_user_reason' not in indexes:
        op.create_index('idx_report_user_reason', 'reports', ['reported_user_id', 'reason'])
...
def downgrade() -> None:
    ...
    if 'reports' in tables:
        ...
        for idx_name in ['idx_report_user_reason', 'idx_report_resolved_created', 
                         'idx_reports_resolved', 'idx_reports_reason', 
                         'idx_reports_reported_user_id', 'idx_reports_reporter_id']:
    ...
    if 'ratings' in tables:
        ...
        for idx_name in ['idx_rating_rated_user', 'idx_rating_trade_rater', 
                         'idx_ratings_trade_id', 'idx_ratings_rated_user_id', 
    ...
    if 'messages' in tables:
        ...
        for idx_name in ['idx_messages_status', 'idx_message_status_sender', 
                         'idx_message_room_id', 'idx_message_room_created', 
                         'idx_messages_sender_id', 'idx_messages_room_id']:
            if idx_name in indexes:
    ...
    if 'friendships' in tables:
        op.execute("DROP TRIGGER IF EXISTS update_friendships_updated_at ON friendships;")
        indexes = [idx['name'] for idx in inspector.get_indexes('friendships')]
        for idx_name in ['idx_friendship_status', 'idx_friendship_users', 
                         'idx_friendships_friend_id', 'idx_friendships_user_id']:
            if idx_name in indexes:
