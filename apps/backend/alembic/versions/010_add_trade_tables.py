...
def upgrade() -> None:
    ...
    if "trades" not in tables:
        op.create_table(
            ...
            sa.Column(
                "initiator_confirmed_at", sa.DateTime(timezone=True), nullable=True
            ),
            sa.Column(
                "responder_confirmed_at", sa.DateTime(timezone=True), nullable=True
            ),
    ...
    indexes = (
        [idx["name"] for idx in inspector.get_indexes("trades")]
        if "trades" in tables
        else []
    )
    ...
    if "idx_trades_initiator_created" not in indexes:
        op.create_index(
            "idx_trades_initiator_created", "trades", ["initiator_id", "created_at"]
        )

    if "idx_trades_responder_created" not in indexes:
        op.create_index(
            "idx_trades_responder_created", "trades", ["responder_id", "created_at"]
        )
    ...
    item_indexes = (
        [idx["name"] for idx in inspector.get_indexes("trade_items")]
...
def downgrade() -> None:
    ...
    if "trade_items" in tables:
        op.drop_index(
            "idx_trade_items_card_id", table_name="trade_items", if_exists=True
        )
        op.drop_index(
            "idx_trade_items_trade_id", table_name="trade_items", if_exists=True
        )
    ...
    if "trades" in tables:
        ...
        op.drop_index(
            "idx_trades_responder_created", table_name="trades", if_exists=True
        )
        op.drop_index(
            "idx_trades_initiator_created", table_name="trades", if_exists=True
        )
